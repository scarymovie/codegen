...
-   `make generate-asyncapi`: Генерирует код для работы с сообщениями на основе спецификаций `*.asyncapi.yaml` из директории `api/`.
...
-   `api/`: Содержит файлы спецификаций OpenAPI (`.openapi.yaml`) и AsyncAPI (`.asyncapi.yaml`).
